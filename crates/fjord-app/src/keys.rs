...
//   Keybindings   normal + player KeyMaps; user JSON merges on top of defaults
//   AppMode       active UI mode — derived from AppState flags at call site
//   default_keybindings  hardcoded defaults; user keybindings.json overrides
...
///
/// Slint delivers printable characters (e.g. "a") and named keys as their
/// PUA codepoint strings (e.g. `key::UP`).  Modifier bools come from
/// `event.modifiers`.  `meta` is intentionally omitted — Slint rarely uses it
/// and we don't have any meta-modified bindings.
#[derive(Debug, Clone, PartialEq, Eq, Hash, Serialize, Deserialize)]
pub struct KeyCombo {
    pub key:   String,
    pub shift: bool,
...
///
/// User's `~/.config/fjord/keybindings.json` is merged on top of
/// [`default_keybindings`]: per-field entries from the file replace the
/// corresponding default entry while all other defaults remain.
...
/// Used by the dispatch function to decide how to interpret each `Action`.
#[derive(Debug, Clone, PartialEq, Eq)]
#[allow(dead_code)]
pub enum AppMode {
    /// Login screen visible — reject all key events.
    Login,
    /// Context menu overlay — Up/Down/Enter/Close only.
    ContextMenu,
    /// Series screen visible.
    Series,
    /// Series screen, season tab row focused.
    SeriesSeasonRow,
    /// Detail page visible.
    Detail,
    /// Player fullscreen, track/panel panel open (S/A/V).
    PlayerPanel,
    /// Player fullscreen, no panel open.
    Player,
    /// Library grid, search field focused.
    LibrarySearch,
    /// Library grid, poster grid focused.
    Library,
    /// Browse screen, search field focused.
    BrowseSearch,
    /// Browse screen, list or sidebar focused.
    Browse,
    /// Settings screen (active-nav == 10).
    Settings,
    /// Home / Movies / TV dashboards and sidebar (the default mode).
    Dashboard,
}

// ── Default keybindings ───────────────────────────────────────────────────────

/// Returns the hardcoded default bindings.
///
/// Call this once at startup, then merge the user's `keybindings.json` on top
/// so that only explicitly overridden entries differ.
...
fn default_player_map() -> KeyMap {
    ...
    // Seek — arrow keys shadow the normal Up/Down/Left/Right navigation
    ...
    // Stats and track panels (shadow NavSettings / OpenDetail in player)
    ...
    // Digit seek (shadow NavHome / NavMovies / NavTV in player mode)
...
/// Main entry point for keyboard dispatch.
///
/// Called from the Slint `handle-key` callback (UI thread).  Returns `true`
/// if the key was consumed (`EventResult::Accept`) or `false` to reject it.
///
/// `repeat` mirrors `KeyEvent.repeat` from Slint: if `true`, the key is being
/// auto-repeated by the OS.  Used to guard "tap Left exits sidebar" so a held
/// Left doesn't chain back to the sidebar after reaching card 0.
pub(crate) fn handle_key(
    ...
    // ── Context menu ──────────────────────────────────────────────────────
    {
        ...
        if g.get_show_context_menu() {
            let Some(action) = action else { return true; }; // swallow unbound keys
            return match action {
                ...
                _ => true // swallow all other keys while context menu is visible
            };
        }
    }
    ...
    match &action {
        Action::Fullscreen => {
            use slint::Global; crate::AppState::get(window).invoke_toggle_fullscreen(); return true;
        }
        Action::Quit => {
            use slint::Global; crate::AppState::get(window).invoke_quit(); return true;
        }
        Action::OpenBrowse => {
            use slint::Global;
            let g = crate::AppState::get(window);
...
fn dispatch_player(action: Action, window: &crate::MainWindow) -> bool {
    ...
    // Escape/Backspace: close panel or stop playback
    if action == Action::Back {
    ...
    // Panel Up/Down/Confirm — intercepted when a panel is open
    if panel != 0 {
        match action {
            Action::Up => {
            ...
            _ => {} // fall through to main player controls
        }
    }
...
fn handle_library_search(key: &str, ctrl: bool, window: &crate::MainWindow) -> bool {
    use slint::Global;
    let g = crate::AppState::get(window);
    if ctrl { return true; } // swallow modifier combos (Ctrl+C etc.)
    match key {
        k if k == key::ESCAPE => {
            g.invoke_library_search_clear();
            g.set_library_header_focused(false);
...
/// Returns `Some(handled)` if the settings block handled the key,
/// `None` if the key should fall through to the Up/Down sidebar nav.
...
/// Returns `true` if `key` is a Slint named/navigation key that should be
/// swallowed in search field mode without being appended to the query.
fn is_navigation_key(key: &str) -> bool {
    let Some(ch) = key.chars().next() else { return true; };
    // Slint named keys start at U+E000 (PUA); control chars < U+0020
    (ch as u32) >= 0xE000 || ch.is_control()
}

/// Returns `true` if `key` is a printable user-typed character.
