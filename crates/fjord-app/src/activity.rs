...
//   ActivityClock         lock-free, Clone-able "when did the user last do
//                         anything" timestamp — shared by the keyboard path
//                         (main.rs::on_handle_key), the winit-level mouse
//                         tap below, and profile::wire_idle_lock_timer's own
//                         idle check.
//   FjordActivityHandler  CustomApplicationHandler impl, registered once via
//                         slint::BackendSelector at the top of main(), before
//                         MainWindow::new() — observes every raw
//                         CursorMoved/MouseInput/MouseWheel winit event
//                         BEFORE Slint's own hit-testing/dispatch, so unlike
//                         the old AppState.record-activity() mechanism (fed
//                         from exactly 3 Slint TouchAreas, replaced by this
//                         same-day, event-loop branch) it sees mouse
//                         activity anywhere on the window — including over a
//                         MediaCard/FjordButton/NavItem that would otherwise
//                         swallow it. Pure observation: always returns
//                         EventResult::Propagate, Slint's own dispatch is
...
pub(crate) struct FjordActivityHandler {
    pub(crate) clock: ActivityClock,
}

impl CustomApplicationHandler for FjordActivityHandler {
    fn window_event(
        &mut self,
