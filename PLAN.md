...
- [ ] **hdr branch, Stage 3: real Wayland color-management negotiation (2026-09-14).** With the "HDR passthrough" toggle (Settings → Video) ON, play a real BT.2020/PQ HDR10 title — confirm the stats overlay's new "HDR" row goes `Negotiating…` → `Active (HDR10)`, `fjord.log` shows the negotiated parameters (`hdr worker: negotiated HDR10 image description — ...`) with no fatal Wayland errors, and — the one thing nothing software-side can substitute for — the TV's own HDR indicator actually engages. Then, same toggle still ON: play ordinary SDR content and confirm the row reads `Not applicable` with no Wayland calls attempted (check `fjord.log` for the absence of any `hdr worker: negotiated...` line). Then toggle it back OFF and replay the same HDR title — confirm the row reads `Disabled` and playback is byte-for-byte identical to before this stage existed. If reachable, also try genuine HLG content — expect `Not applicable` (this exact KWin version doesn't advertise the `Hlg` named transfer function; a documented, deliberate v1 scope cut, not a bug). Whatever this reports determines whether Stage 4 (mpv-side real HDR output — widening the FBO, `--target-trc`/`--target-prim`/`--target-peak`) is safe to build next; per its own doc comment in `hdr.rs`, Stage 4 must never ship ahead of a confirmed-live Stage 3 success.
...
- **Real HDR passthrough (`hdr` branch) — Stages 1-3 implemented, Stage 3 awaiting live confirmation; Stage 4 still deferred.** `Config.device.target_colorspace_hint` ("HDR passthrough" in Settings) does nothing at all under Fjord's actual render path via mpv's own mechanism — mpv's manual states `--target-colorspace-hint` "Requires a supporting driver and `--vo=gpu-next`", but Fjord uses `vo=libmpv`. Real HDR passthrough means Fjord doing its own Wayland `color-management-v1` negotiation directly against its own surface, independent of mpv — see CLAUDE.md's HDR section for the full trace.
  ...
  - **Stage 3 (2026-09-14)** — the real negotiation itself: for genuinely HDR10 (PQ+BT.2020) content, builds a real `wp_image_description_v1` (validated mastering-luminance/CLL/FALL from mpv's own per-file HDR10 metadata) and applies it to Fjord's surface via `wp_color_management_surface_v1.set_image_description`. Reuses the existing "HDR passthrough" Settings toggle (default off — no existing install's behavior changes until opted in) and adds a live on-screen "HDR" row to the stats overlay so this is actually verifiable without tailing `fjord.log` mid-playback. `cargo build`/`clippy --workspace --all-targets`/`test --workspace` clean — **not yet live-tested**, see this doc's own Live-test checklist entry above for the exact verification steps. HLG content and non-named/custom primaries are a deliberate v1 scope cut (this compositor doesn't advertise `Hlg`), not a bug.
  - **Stage 4 (mpv-side real HDR output — widen the FBO to 10-bit/float, set `--target-trc`/`--target-prim`/`--target-peak`) remains explicitly deferred**, strictly gated on Stage 3 being confirmed working live first — shipping it without that confirmation would very plausibly make HDR content look *worse*, not better (real PQ-range values sent to a compositor still treating the surface as ordinary sRGB).
