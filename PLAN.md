...
- **Bonfire/JellyProfiles integration — in progress, multi-session.** Full design approved and saved (plan file `groovy-stirring-phoenix.md`), documented in outline in this file's history and in full in CLAUDE.md as each phase lands.
  ...
  - **Phase 1, remaining scope — now exactly one item**: the async-result session-guard audit (checking whether `invoke_from_event_loop` closures across Detail/Series/Season/Collection/Album/Artist/Person/Discover/poster-loading/prewarm need a `session_current(...)` guard now that a genuine mid-fetch profile switch is a real, reachable scenario).
