...
- **Bonfire/JellyProfiles integration — in progress, multi-session.** Full design approved and saved (plan file `groovy-stirring-phoenix.md`), documented in outline in this file's history and in full in CLAUDE.md as each phase lands. **Phase 0 (Settings screen data-driven rewrite) shipped 2026-08-07** — see CHANGELOG.md and CLAUDE.md's Settings section for the design. Build/clippy/test clean; **not yet live-tested** — given the size of this rewrite (~90 rows across 7 sections, all keyboard nav re-derived from a new per-section row-list builder), this is the highest-priority item to check on the HTPC or dev machine before trusting it: every section's Up/Down/Enter/Right nav, every dropdown (static and the 7 dynamic ones), the two bespoke action rows (Connect/Disconnect, Manage Blocklist) plus the new Sign Out row in the new Profiles section, and that mouse clicks and keyboard nav agree on focus (scroll-to-view position in particular, since that's the one new mechanism — `settings-focused-visual-index` — that didn't exist before). Phase 1 (Foundation + profile switching) is next, not yet started.
