...
- **Real HDR passthrough (`hdr` branch) — Stages 1-3 implemented, Stage 3 awaiting live confirmation; Stage 4 still deferred.** `Config.device.target_colorspace_hint` ("HDR passthrough" in Settings) does nothing at all under Fjord's actual render path via mpv's own mechanism — mpv's manual states `--target-colorspace-hint` "Requires a supporting driver and `--vo=gpu-next`", but Fjord uses `vo=libmpv`. Real HDR passthrough means Fjord doing its own Wayland `color-management-v1` negotiation directly against its own surface, independent of mpv — see CLAUDE.md's HDR section for the full trace.
  ...
  - **Stage 4 (mpv-side real HDR output — widen the FBO to 10-bit/float, set `--target-trc`/`--target-prim`/`--target-peak`) remains explicitly deferred**, strictly gated on Stage 3's negotiation being confirmed working live first (now satisfied) — shipping it without that confirmation would very plausibly make HDR content look *worse*, not better (real PQ-range values sent to a compositor still treating the surface as ordinary sRGB). **Also gates the reverse test**: switching the output into real HDR mode and playing HDR content is deliberately being held off until Stage 4 exists too — with only Stage 3 shipped, Fjord's actual pixel data is still plain SDR-range (mpv's `--target-trc=auto` default tone-maps to gamma 2.2 before `create_fbo()` ever sees it), so a compositor treating that surface as genuine PQ content while the pixels are still SDR-range would show a real, badly wrong (crushed/too-dark) picture — not a useful signal about anything. Once Stage 4 lands: toggle the output into HDR (`media_display_sync` or the manual KDE toggle), play HDR content with the Fjord toggle on, and confirm the TV's own indicator engages and the picture looks correct — the one thing nothing software-side can substitute for.
