...
- [ ] **Step 3 — Collection row**: **blocked on field-name verification** — the exact Jellyfin field that identifies a movie's parent BoxSet (`CollectionId`? `ParentId`?) must be confirmed against a real API response before implementing. Once confirmed: fetch sibling items (`GET /Users/{userId}/Items?ParentId={boxsetId}&SortBy=ProductionYear`); show as "Part of [Collection Name]" `SectionRow` in `detail.slint` — Enter opens that movie's detail. Movies only.
