...
- [ ] **#44 — No mouse hover highlight on dashboard/library items** — `MediaCard` hover highlighting likely has the same Slint reactive dependency-tracking issue as the settings panes. `has-hover` in a ternary else-branch not read unconditionally. (`home.slint`, `browse.slint`)
