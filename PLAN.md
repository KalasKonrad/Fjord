...
- [ ] Bonfire Phase 5 (cross-household groups) — implemented 2026-08-29, `cargo build`/`clippy --workspace --all-targets`/`test --workspace` clean. Fixed the same day from live testing: a `BonfireGroupStatus` deserialize crash on open, the join-code field being completely untypeable with the on-screen keyboard disabled (a real gap unique to this field — every other hand-drawn field already had its own direct-typing fallback), and a pre-existing (since Phase 2) shared-counter bug that visibly flashed the wrong Settings row whenever Enter was pressed inside this or Manage Profiles. None of the 3 fixes yet re-confirmed live. The actual cross-household connect/switch flow is still entirely unverified (needs two real master accounts). See CLAUDE.md's own dated section for the full design, the real correctness bug found and fixed while designing it (a foreign group account was one code path away from being silently merged into the discovering master's own account, or from getting silently "repaired" back to a plain account on the next restart), and the priority-ordered live-verification checklist.
