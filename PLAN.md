...
- [x] **CR10-4** *(fixed 2026-07-06)* Quit keybinding destroyed by queue-panel binding — in `default_normal_map`, `q`/`Q` → `OpenQueuePanel` (Phase 51) overwrote `q`/`Q` → `Quit` in the same HashMap; fresh installs / reset-to-defaults had no Quit key at all. Resolution: Quit's default is now **Ctrl+Q** (desktop convention); plain `q`/`Q` stays with the queue panel as Phase 51 intended. The loading-overlay quit bypass in `handle_key` updated to require Ctrl. Sidebar Quit item unchanged for remote/HTPC use. CLAUDE.md shortcut docs updated. **Follow-up:** Quit is now a global pre-dispatch in `handle_key`, so Ctrl+Q works from every mode — the old per-screen Quit arms only covered dashboard/settings/series/season/detail/person, which is why q/Q never worked in the library grid, browse, player, or music screens. Note: users with a pre-Phase-51 `keybindings.json` keep whatever they had (file replaces defaults).

**High — broken features**

- [x] **CR10-5** *(fixed 2026-07-06 — verify on HTPC)* Shift+letter bindings never matched — `main.slint` passes `event.modifiers.shift` into the KeyCombo lookup, but uppercase defaults are stored with `shift: false` (`KeyCombo::plain("Z")`). Shift+z produced `{key:"Z", shift:true}` → no match → `Z`/`X` (sub/audio delay **decrease**) were dead from the keyboard. Added `lookup_action()`: exact match first (explicitly shifted bindings still win), then a shift-stripped retry for single printable chars only (named keys like shift+Left stay distinct). Applied to normal-map, player-map, and the music-bar PausePlay fallback lookups. **Runtime-verify Z/X during playback on the HTPC.**
- [x] **CR10-6** *(fixed 2026-07-06)* Queue panel never showed "Add to Queue" items and `queue-count` had two meanings. `push_queue_display` now renders playlist rows followed by context-menu queue rows (`QueueEntry.is-queued` — "· Queued" subtitle in subtle colour, display index continues) and owns `queue-count` via new `playback::upcoming_count` (playlist tracks after current + queued items); all scattered `set_queue_count` writers removed. `queue-jump`/`queue-remove` map panel rows to the right collection (queued rows play-now-with-removal / remove-from-queue).
- [x] **CR10-7** *(fixed 2026-07-06)* Enqueuing a Series card created an unplayable queue item (`direct_play_url(series_id)` has no stream). `queue_from_context_menu` (shared body for both queue callbacks, with new `enqueue_item` helper) now resolves Series cards to their next unwatched episode via `get_next_up_for_series` before enqueueing; fully-watched series or fetch failure shows a toast. `wire_queue_callbacks` gains an `rt_handle` param for the async resolution.
- [x] **CR10-8** *(fixed 2026-07-06)* "Play Next" was broken under shuffle after a few tracks — the shuffle_order insert was hardcoded to slot 1, which is only correct immediately after toggling shuffle; once playback advanced to shuffle position k, the inserted track sat behind the cursor and never played. `enqueue_item` now inserts right after the current item's position in shuffle_order.
- [x] **CR10-9** *(fixed 2026-07-06)* Wrong nav guard in `push_decoded_series` — refreshed the library grid on `active-nav == 2` (Movies) instead of 1 (TV): TV grid showed poster-less cards after a cold open, Movies grid got a spurious refresh that re-shuffled sort=Random. Guard corrected to `== 1`.
