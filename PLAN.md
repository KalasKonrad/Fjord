...
- [ ] Profile switch shows the picker's own "Signing in…" spinner for the full 1-3s gap, not a blank dashboard
- [ ] Outgoing profile's content clears immediately on switch — no stale content lingering; the queue panel/playback overlay reset correctly if a switch happens mid-queue
- [ ] A wrong PIN, or any other failed switch, leaves the outgoing session fully intact and usable
- [ ] A genuinely multi-account install (2+ accounts, at least one `remember_login` off) shows the account picker at startup instead of silently resuming — note: even with `remember_login` ON, "Always Ask" account launch policy still means picking an account every launch; `remember_login` only controls whether a *resolved* account needs its password re-entered
- [ ] Unchecking "Remember this login" forces a real password re-entry next launch (not a PIN, not a silent resume); server + username are pre-filled, only the password needs retyping
...
- [ ] A single-profile account's tile switches directly, no intermediate Profile Picker flash
- [ ] Account launch behavior / Default Account behave as labeled after a real restart
- [ ] `config.json` no longer picks up a self-referencing master-profile entry after a normal session
- [ ] A stale/invalid stored token (401 on switch) — Add Account is confirmed as the way back in

### ProfileEditScreen — full D-pad keyboard navigation
- [ ] Escape correctly exits text-editing (Name/Blocked tags/Allowed tags) without getting stuck — highest-risk item, first use of `LineEdit.key-pressed` in this codebase
- [ ] Tab still moves natively between the 3 text fields
...
- [ ] Both PIN pads' 12-key grid nav + Enter-confirm match the mouse path exactly
- [ ] Both dropdown popups (Max parental rating / Auto-lock) open/scroll/confirm/cancel correctly
- [ ] Save/Cancel/Delete all reachable via the button row, including Delete's conditional presence
- [ ] A mouse click followed immediately by a keyboard press acts on what was actually clicked
- [ ] Whole-screen auto-scroll — accepted to ship imprecise for the two checklist zones specifically; the focus *ring* should always be correct regardless of scroll timing

### Native profile management (Manage Profiles / Edit Profile)
- [ ] Settings → Profiles → Manage Profiles reachable, master accounts only
- [ ] Create a new sub-profile end to end (name, avatar, PIN, rating, libraries, tags, lockout, LAN bypass, device whitelist, master PIN)
- [ ] Edit an existing sub-profile — every field pre-fills correctly except parental rating (known gap, Bonfire's `/list` doesn't return it)
- [ ] Setting a PIN via Manage Profiles → Edit → Save immediately shows the lock icon + opens the PIN pad on the next picker visit, no restart needed — same for a freshly created sub-profile appearing right away
- [ ] Delete a sub-profile
- [ ] Sidebar quick-menu's "Edit My Profile" (master only): row only shows for a master, no Delete button, self-edit save round-trips against a real Bonfire `/update` call, sidebar tile/settings dropdowns update immediately
- [ ] Bonfire API surface beyond profile list/switch/create/update/delete (already exercised live) — the ~17 remaining methods (libraries/devices/group/preferences/admin) are still unverified against a real server

### Discover / Seerr
- [ ] Discover Filters: Genre/Provider popups populate from a real instance; multi-select genuinely broadens results (OR, not AND); Sort/Rating/Year narrow/reorder correctly; popup sizing fits real content
- [ ] Seerr Blocklist: add/remove across the Discover context menu + RequestDetailScreen + collection bulk-blocklist; Manage Blocklist screen pagination/remove; blocklisted items stay gone from Discover *and* the dashboard Watchlist/Coming-Up rows; `⛔` glyph renders on the HTPC
- [ ] Watchlist/Calendar: marking watched removes from the watchlist (except a still-Continuing series); re-adding a watched item marks it unwatched; calendar month-grid renders correctly at real font metrics; day-popup keyboard nav; dashboard Watchlist/Coming-Up rows keyboard-reachable; star badge doesn't collide with a progress bar
- [ ] RequestDetailScreen/RequestOptionsOverlay: loading state, focus rings, dialog layout, remembered Quality/Tags/Profile choices, auto-watchlist-on-request
...
- [ ] Deep Seerr integration remaining opens: Series' own `spawn_recommended`, Missing Seasons per-season pill correctness + Request Options preselect, Person Other Work TMDB-id accuracy, cache clear on Seerr disconnect/sign-out
- [ ] Discover search grid: rapid typing / pagination doesn't flash or re-fade already-loaded posters
- [ ] Discover posters: a blocklisted item no longer misaligns posters on the rest of that row
- [ ] Watchlist-toggle toast mystery — still genuinely unresolved (confirmed not a silent-return bug; root cause unknown)

### Playback
- [ ] A real network outage recovers via stream reload instead of stalling or misadvancing to the wrong episode; seeking/pausing no longer false-triggers the stall watchdog
- [ ] Skip-segment fade-to-black: duration setting works; SPDIF passthrough mute during the fade doesn't cause AVR relock issues (genuinely uncertain, not just unverified)
- [ ] Pausing during the Up Next banner / ask-timed skip overlay correctly freezes the countdown
- [ ] Fresh app launch's first playback shows video immediately, no restart needed (VO-init race fix)
- [ ] Season-tab right-click/`C` context menu works; Mark Watched cascades to episodes via the WS delta-sync path

### Misc UI
- [ ] Browse All still shows all items after leaving and returning to the tab (not empty)
- [ ] LibraryGrid/BrowseItem stale-highlight fixes hold up when moving to the search/sort/filter zones
- [ ] Anime-provider HTML bios render as clean text, not raw markup
- [ ] White focus-border ring reads clearly across all unified sites on a real display
- [ ] Missing Seasons row shows "Upcoming · date" instead of "0 episodes" for an unaired season
- [ ] Login: a bare hostname tries HTTPS-then-HTTP correctly; an explicit scheme is never second-guessed
- [ ] Pause/play flash icon renders correctly (not tofu) on the HTPC specifically
