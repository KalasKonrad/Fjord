...
- [x] Bonfire Phase 5 follow-up (2026-08-31): "Switch Profile" now merges every Bonfire-linked household directly into one sectioned picker (own household + one section per linked household, each including that household's own master tile) instead of requiring a separate "Switch Account" step — matching Bonfire's own official "Who's Watching?" UI. Two `ExitPlanMode` rejections and an independent second review pass caught real bugs before implementation (a control-flow bug that would've silently dropped the new linkage field for exactly the case it exists for, a security-relevant `group_into_accounts` sort-key bug, a missed second `ProfileTile` constructor, a wrong reclamp formula, a centering-formula regression). `cargo build`/`clippy --workspace --all-targets`/`test --workspace` clean (16/16 tests, including a new regression test). Live-reported the same day ("i spot i bug;)") via a screenshot showing "Your Bonfire" duplicated as a bogus "Anton's Bonfire" section — Bonfire's own `/list` also returns your own household's sub-profiles, and the new linked-roots tracking recorded your own account root as if it were a different linked household; fixed by excluding it. See CLAUDE.md's own dated section for the full design. Not yet re-confirmed live after the duplicate-section fix. A third, distinct bug the same day: kicking an account via Jellyfin's own web UI (not Fjord) left the cold-start picker still showing the kicked household until the user logged into any account — no live session exists at cold start to check Bonfire against, so it always showed last-saved-to-disk data. Fixed with a new `sync_all_known_accounts_in_background`, fired right before any cold-start picker shows, using every known account's own stored token — needed a deliberate, carefully-scoped widening of `sync_bonfire_subprofiles`'s own session guard to work at all (it used to unconditionally discard when no session existed yet, which is exactly the case at cold start). `cargo build`/`clippy --workspace --all-targets`/`test --workspace` clean. Not yet live-tested.
