...
- **Bonfire/JellyProfiles integration — in progress, multi-session.** Full design approved and saved (plan file `groovy-stirring-phoenix.md`), documented in outline in this file's history and in full in CLAUDE.md as each phase lands.
  ...
  - **Phase 1, remaining steps — not yet started**: the async-result session-guard audit, and the launch-policy Settings row (Always Ask/Remember Last/Default Profile UI — the `DeviceConfig` fields backing it already exist from step 1, but nothing in Settings can change them yet, so every install is effectively pinned to "always ask" once it has 2+ profiles).
