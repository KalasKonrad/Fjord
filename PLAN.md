...
| 27 — UI polish: icon button centering/size, bigger spinner, portrait preload, progress bar (2026-06-23) | (1) `IconCircleButton.Text` given explicit `width/height: parent.width/height` so the ♥/✓ character is truly centred; font-size bumped from 16 px → 20 px, weight 600 for bolder look. (2) `LoadingSpinner` dots enlarged 10 px → 14 px, spacing 10 → 14 px, height 48 → 64 px for visibility. (3) Cast portraits now fetched **before** the page is shown in both `detail.rs` and `series.rs` `spawn_main`: parallel `JoinSet` collects all portrait buffers into a `Vec<Option<SharedPixelBuffer>>` (Send), then a single `invoke_from_event_loop` builds the full `Vec<CastMember>` with photos ready — no trickle-in after page appears. (4) `app-loading-progress: float` property added to `AppState`; `spawn_main` emits 0.5 progress after metadata+images; final invoke sets page visible + clears to 0. `main.slint` loading overlay now shows a 240 px progress bar (accent fill, `animate width 400 ms`) below the spinner. Progress reset to 0 on cancel (keys.rs) and on each new `open_detail`/`open_series_screen`. |
