...
- [x] **#17 — Make icon backgrounds transparent** — removed solid white/near-white background `<path>` from `fjord_01/03/05/06/09.svg`. Icons `02/04/10` have intentional dark backgrounds; `07/08` have white corner fills for their rounded-square shape (left as-is).
- [x] **#18 — Add icon next to "Fjord" name in sidebar** — `app-logo-idx` (1–10) set randomly at startup via subsecond nanoseconds; sidebar header now shows `Image` + VerticalLayout with "Fjord"/"Jellyfin" text. All 10 SVGs embedded via `@image-url()` ternary chain. (`layout.slint`, `app_state.slint`, `main.rs`, `assets/`)
...
- [x] **#41 — Skip intro prompt never shows** — Root cause: race condition in `start_playback`. The Intro Skipper HTTP tasks were spawned before `Player::new`, so on a fast LAN the response could arrive and set `vs.intro_timestamps = Some(ts)` before the `Player::new` block ran — which then reset it to `None`. Fix: reset `intro_timestamps / intro_skip_shown / credits_start` to `None` *before* spawning the fetch tasks so any fast response survives; remove the redundant reset from the `Player::new` init block (non-Episode items still reset there). Also elevated log level from `debug!` to `info!`/`warn!` for fetch outcomes so HTPC logs confirm whether timestamps arrive, and tightened HTTP error handling (404 → `Ok(None)` silently; other HTTP errors → `warn!` with status + body snippet). `Valid` serde field now defaults to `true` to tolerate API responses that omit it. (`fjord-api/src/client.rs`, `fjord-api/src/models/intro.rs`, `fjord-app/src/playback.rs`)
- [x] **#44 — No mouse hover highlight on dashboard/library items** — Two issues: (1) `MediaCard.poster-rect.background` changed colour on hover but was completely hidden by the poster `Image` which fills the rectangle with `image-fit: cover`. Fix: keep `background: Theme.surface-raised` always and add an animated `#ffffff1a` (10% white) overlay `Rectangle` as the last child of `poster-rect`, above the image and all badges, with `opacity: ta.has-hover ? 1.0 : 0.0` and 120 ms animation. (2) `BrowseItem.background` had `ta.has-hover` in the else-branch of a ternary guarded by `selected`, so Slint only tracked `has-hover` when `selected` was false. Fix: block expression `{ let hov = ta.has-hover; selected ? … : (hov ? …) }` to force unconditional read. (`widgets.slint`)
- [x] **#43 — Keyboard still focused on browse search after mouse click on item** — `BrowseItem.clicked` set `current-item` but left `browse-header-focused = true`, so the keyboard handler kept routing events to the search field. Fix: add `AppState.browse-header-focused = false` to the clicked handler. (`browse.slint`)
- [x] **#42 — Can't click search field with mouse in Browse All** — the search bar was a `Rectangle` with `Text` elements but no `TouchArea`, so mouse clicks fell through with no handler. Fix: add a `TouchArea` inside the search Rectangle that sets `browse-header-focused = true` and `current-item = -1` on click. (`browse.slint`)
