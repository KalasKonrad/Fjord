...
- [x] **#19 — Backspace/Escape behaviour in player** — Fixed: Backspace/Escape in fullscreen player now always minimize (close open panel first if one is open, then `invoke_minimize_player`). Stop removed from keyboard — accessible only via controls bar "■ Stop" button and the new "■" stop button in the Now Playing sidebar card. `settings_video_behind` conditional removed from `dispatch_player`. (`keys.rs`, `layout.slint`)
