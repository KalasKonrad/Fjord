...
- [ ] **Step 3 — Playback section**: per-segment skip mode (`always-ask` / `always-skip` / `never-skip`) in `Config` for each of Intro, Recap, Preview, Commercial; `playback.rs` reads mode per segment; toggles in Settings → Playback.
