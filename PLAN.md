...
| 24 — Back button + series header button keyboard nav (2026-06-22) | `series-focused-btn: int` (-1=not in header, 0=Back, 1=♥, 2=✓Watched): Up from Next Up or season row → header buttons; Left/Right cycle; Enter activates; Down returns to Next Up or season row. Detail page: `detail-focused-btn = -1` = Back; Left from Play (0) → -1; Enter on -1 = back action. Season screen: `season-focused-back: bool`; Up from episode row → Back focused; Down/Right exits back to episode row; Enter closes season. All three Back buttons gain `kbd-focused` ring. |
| 23 — Series detail UX polish (2026-06-22) | (1) "✓ Watched" / "✓ Unmark" button added to series detail alongside ♥ fav; `toggle-series-played` callback wired in `main.rs` (mirrors fav toggle pattern). (2) `PosterBlock` in `widgets.slint` extended with `has-played`, `resume-pct`, `unplayed-count` badge overlays (✓ circle, progress bar, count pill) — matching `MediaCard` badge logic. Series detail poster uses `PosterBlock` (previously raw Rectangle) and passes `series-has-played` / `series-unplayed-count`. Movie detail poster passes `detail-has-played` / `detail-resume-pct`. (3) Season row focus indicator: bottom border changes from 1px `Theme.border` to 2px `Theme.accent` when `series-in-season-row = true`. (4) C key on series Next Up card opens context menu. (5) Default focus when opening series screen is episode row (`series_in_season_row = false`); `spawn_next_up` steals focus to Next Up when data arrives (if user hasn't navigated away). |
| 25 — Crash fix: series screen "Recursion detected" (2026-06-23) | Root cause: `fl.kb-y` ChangeTracker (in `user_init`) reads `kb-y` → reads anchor element Y positions → triggers the content VerticalLayout layout cache evaluation (marked BEING EVALUATED) → `ensure_updated()` on the season tabs conditional → season tabs `user_init` reads `kb-x` → `kb-x` reads `self.width` (the conditional Rectangle's allocated width, derived from that same content layout cache) → "Recursion detected" panic. Fix: replace `self.width` with `root.width` in the season tabs `kb-x` formula in `series.slint`. `root.width` is the SeriesScreen root's width set by the outer parent, never derived from the internal content layout cache. Both values are always equal because the Rectangle fills the full screen width. |
| 26 — UI polish: backdrop header, icon circle buttons, ends-at, load-then-show (2026-06-23) | (1) **Ends at** moved out of the meta chip row (year · rating · runtime) — now shown as "Ends HH:MM" text below the action buttons on the detail page; `detail-ends-at` new AppState property. (2) **Fav + Watched buttons** replaced with `IconCircleButton` component (38 px circle, gray=inactive, accent=active) on both movie and series detail — ♥ and ✓ icons. (3) **Backdrop extends to header content**: removed standalone `BackdropHero` from `detail.slint` and fixed-height backdrop Rectangle from `series.slint`; header section is now a single `Rectangle` with the backdrop `Image` filling it (height driven by the inner `HorizontalLayout` content) + a dark gradient overlay; muted text brightened to `#cccccc` when backdrop is present. (4) **Load-then-show**: `open_detail` and `open_series_screen` no longer set `show_detail`/`show_series = true` immediately; instead they set `app-content-loading = true` and let `spawn_main`'s `invoke_from_event_loop` set both the screen flag and clear the loading flag once metadata + poster + backdrop are ready; `main.slint` shows a semi-transparent overlay with `LoadingSpinner` while loading; `keys.rs` blocks all keys during load (Escape/Back/Quit cancel); `detail-loading` UI guards removed since page is never visible while loading. |
...
Issue
1 The movie details ends at shuld not be in the meta data culd it be in the play button or below it and if there is a resume it shuld also be in that or belove that
2 for the series detail thet mark as watch button has a checkmark then watch/unmark and in the movie detal it state mark as watched then if it is watched it get the checkmark then watched, instead of tex why not a cirlve wit a checkmar in that is gray if its not watched and blue when it is watched?
3 the favorite in series detail and movies detail is a blue button with a unfiiled haert when not favorite and a blue button with a filled heart when favorit, think it shulb be gray when its not favorit
4 the heather image for series and movie detail culd be all the way down to the cast row, and to be sure the text is readable for the poster and things we culd make it a semi transperand gray background and make te text whiter
5 and insted of having elements to load in dynamicly when the user already is on the page we shuld wait on the previus page and show a loading symbol an change to the page when everything is loaded
