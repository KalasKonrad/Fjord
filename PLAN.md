...
- [ ] Bonfire Phase 5 (cross-household groups) — implemented 2026-08-29, `cargo build`/`clippy --workspace --all-targets`/`test --workspace` clean. The screen itself is now confirmed to open correctly against a real server (a real `BonfireGroupStatus` deserialize bug — `AllowHouseholdLanBypass`/`IsAdministrator`/`HasPin` all genuinely missing from the plugin's own live response — found and fixed the same day), but the actual cross-household flow is still unverified (needs two real master accounts). See CLAUDE.md's own dated section for the full design, the real correctness bug found and fixed while designing it (a foreign group account was one code path away from being silently merged into the discovering master's own account, or from getting silently "repaired" back to a plain account on the next restart), and the priority-ordered live-verification checklist.
