...
- [ ] **#43 — Keyboard still focused on browse search after mouse click on item** — after mouse-clicking an item from browse search results, keyboard focus remains on the search `TextInput`, so arrow keys type characters rather than navigate. Need to clear `browse-header-focused` on item click. (`browse.slint`, `browse.rs`)
- [ ] **#42 — Can't click search field with mouse in Browse All** — mouse click on the browse search field doesn't transfer focus to it. (`browse.slint`)
