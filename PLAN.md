...
- [x] **#29 — Subtitle language preference setting** — `sub_lang` added to `Config` (serde default = ""). Settings → Player → "Preferred subtitle language" dropdown (row 12: "", "en", "de", "fr", …, 20 options). At playback start, if no sub track is active and `sub_lang` is set, the first track whose `lang` starts with `sub_lang` (case-insensitive) is auto-selected via `player.set_sub_track()`. Keyboard Left/Right cycle the dropdown. (`config.rs`, `app_state.slint`, `settings.slint`, `settings.rs`, `playback.rs`)
