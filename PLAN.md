...
- [ ] Discover search grid doesn't flash or re-fade already-loaded posters while typing/paging
  ...
  **FIXED 2026-08-21 (separate bug, same session)**: "all the posters flash every time it loads one item" — even with no new query. Posters for a single search page were being patched into the grid one at a time, each its own separate Slint commit; a page's worth of TMDB fetches commonly lands in a tight burst, so a dozen-plus individual commits fired back to back. Now batched over a short window, matching how the (already-correct) Library grid does its own poster loading.
- [ ] Poster fade-in reads as a smooth fade, not an abrupt flash
  **FIXED 2026-08-21**: 200ms → 400ms for `MediaCard`'s shared poster fade (used app-wide, not just Discover) — still scaled by the user's own animation-speed setting.
