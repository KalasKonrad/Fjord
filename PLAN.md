...
| 34 — Sort, filter, and alphabet scrubber in library grid (2026-06-25) | Client-side sort (Name A-Z/Z-A, Year↓/↑, Random via LCG) and Unwatched/Favorites filters applied to cached `all_movies`/`all_series`. Sort persisted per library type in `Config`. `refresh_library_display` central function rebuilds `library-display` and `library-alpha-offsets` (27-element model, A-Z+#). Sort bar UI (40px strip) with 5 sort pills + 2 filter toggles; Tab to focus, Left/Right nav, Enter applies, Esc exits. Alphabet scrubber: right-edge 22px strip visible only when sort=A-Z and no query; 27 letters each click/key-jumps grid. Letter keys A-Z in library grid mode auto-jump when conditions met. `on_open_library` sets sort from Config synchronously before async work. |
