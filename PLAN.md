...
- **Bonfire/JellyProfiles integration — in progress, multi-session.** Full design approved and saved (plan file `groovy-stirring-phoenix.md`), documented in outline in this file's history and in full in CLAUDE.md as each phase lands.
  ...
  - **Phase 1, remaining steps — not yet started**: cache namespacing (per-profile `movies.json`/etc.), `reset_session_state` extraction, the async-result session-guard audit, the shared plugin-availability registry, the Bonfire `fjord-api` module, the numeric `VirtualKeyboard`, `ProfilePickerScreen`, the launch-policy Settings row, and full session-swap plumbing.
