...
- **Phase 183 (Deep Seerr integration) — live verification.** Calendar ongoing-series, Collection Missing Items, and Missing Seasons keyboard nav are confirmed/fixed. Still open: (1) Series Missing Seasons — per-season pill correctness, Request Options preselect on click; (2) Person Other Work — TMDB person-id resolution accuracy for real actors; (3) Detail/Series Recommended — sensible not-owned recommendations; (4) a Seerr disconnect/reconnect or sign-out clears `person_tmdb_id_cache`/`person_other_work_cache`.
