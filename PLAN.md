...
- [x] PIN-entry modal's Cancel button is D-pad/mouse-reachable and shows a clean single highlight, no double-highlight with the grid behind it
...
- [x] Add Account's Cancel/Back returns to wherever it was opened from, reachable via keyboard, with a label matching what it actually does
  ...
  **FIXED 2026-08-21 (2nd report, "back and quit is not reachable with keybord/dpad")**: neither button was ever a real D-pad zone — the design assumed Escape/Ctrl+Q already covered them, but that's a shortcut, not the button itself being focusable, the same gap already found and fixed once for the account/profile pickers' own Back/Quit buttons. Added two more zones (5=Back when in append mode, 6=Quit always) with Up/Down chaining from Connect.
- [x] "Remember this login" toggle isn't cramped against its own border
  **Attempted 2026-08-21**: 4px padding bumped to 12px — a real screenshot the same day showed this never actually fixed it.
  **FIXED 2026-08-21 (real fix)**: same class of fix as the ProfileEditScreen row below — dropped the `HorizontalLayout` for explicit x/y positioning of the row's children (this row never had that other row's own `width: parent.width` bug, but shared its unproven `HorizontalLayout`-padding mechanism).
- [x] AccountPickerScreen ("Switch Account") has a reachable Back button when opened from a live session, and none at all on a genuine cold start
  **FIXED 2026-08-21**: live-reported directly — this screen had a keyboard-focusable Quit button but no Back button at all, mouse or keyboard, only a silent Escape/Backspace shortcut. Added a real "← Back" button, shown only when reached via "Switch Account" (never at cold startup, matching the report's own stated requirement), with the same D-pad reachability the Quit button already had.
- [x] Profile picker's "Back" button does what "back" should mean: return to wherever you actually came from, not always "Accounts"
...
- [] A sub-profile deleted on the Bonfire server itself eventually disappears from Fjord's own picker too, instead of staying forever as an unreachable, unmanageable ghost tile
  **FIXED 2026-08-19**: confirmed via a live diagnostic against the real server that this was genuinely happening (a deleted "test 2" sub-profile stayed listed in "Who's watching," permanently, since the add-only sync this app shipped with never pruned anything) — `sync_bonfire_subprofiles` now removes a local entry once the server stops reporting it, scoped tightly to that one household so it can never touch an unrelated account
  ...
  It did get removed from the manage profiles but not the switch profiles, did remove test from fjord and test22 from jellyfin web, but both still show up in profile switcher
...
- [x] `home.json` (Continue Watching/Next Up/Recently Added/Favorites) actually updates on disk after a login or profile switch, not just live on screen
...
- [X] The sidebar's own profile tile updates immediately after a switch, and the dashboard lands on Home instead of wherever the sidebar cursor happened to be
...
- [x] "Skip PIN on this network" row's toggle isn't cramped against its own border
  **Attempted 2026-08-19**: padding 4px→12px on a `HorizontalLayout` — didn't fix it.
  **Attempted 2026-08-19 (2nd try)**: dropped the layout for explicit x/y positioning of the row's children — still didn't fix it, per a real screenshot ("ist not missalinged the same way atleast;)").
  **FIXED 2026-08-21 (real root cause, from the screenshot)**: neither prior fix ever touched the actual bug — the row's own `Rectangle` explicitly set `width: parent.width`, which (inside a padded `VerticalLayout`) reads the *unpadded* layout width; every sibling row (the tag inputs, the Auto-lock dropdown) gets the correct padded width automatically by leaving `width` unset. This row alone rendered visibly wider than everything around it. Removed the explicit width — now sized the same way as its siblings.
...
- [x] Discover search grid doesn't flash or re-fade already-loaded posters while typing/paging
  **LIVE-CONFIRMED 2026-08-21**: "wow nice work with the search works like a charm exactly what i wanted" — after the poster-fade removal below.
  ...
  **Attempted 2026-08-21 (separate bug, same session), superseded same day**: "all the posters flash every time it loads one item" — batched poster commits over a 120ms window instead of one per arrival. User pushed back directly: "but culd we not have a stedy strickle but fix the flashes?" / "why do we ned to flash every poster when we trickle in data?" — the batching was solving the wrong variable (commit frequency, not the fade-in motion itself). Reverted to per-arrival commits (no batching, no delay) — see the poster-fade removal below, which is the real fix.
- [x] Poster fade-in reads as a smooth fade, not an abrupt flash
  **Attempted 2026-08-21**: 200ms → 400ms for `MediaCard`'s shared poster fade — did not help (user already had animation-speed at 300%, ~1.2s effective fade, still flashed).
  **FIXED 2026-08-21 (real fix, same day)**: removed the fade-in animation entirely, not tuned it again. Each card's poster now appears at full opacity the instant it arrives — the staggered *arrival* across a grid already reads as progressive; the extra fade-in motion on top of it, repeated per card, was the actual flash regardless of duration.
- [x] Opening a fast-loading Discover item goes straight to content instead of a flash-then-flash (loading spinner, then content) sequence
  **FIXED 2026-08-21**: standard delayed-spinner pattern — the loading overlay now only actually shows if the fetch is still running after 150ms; a fetch faster than that goes straight to content with no visible spinner at all. Scoped to RequestDetailScreen (what was reported), not the other screens sharing the same overlay mechanism.
...
- [x] Dashboard tab switches (Home/TV/Movies/Collections/Music/Settings) don't flash when navigating
  **FIXED 2026-08-21**: researched real UX transition guidance first (NN/g, Material/HIG — 200-300ms is the standard range, and a hard cut into a fade reads as jarring regardless of duration). Confirmed Fjord's dashboard tabs were doing exactly that hard-cut pattern (old screen destroyed instantly, new one fades in alone) unlike overlay screens (Detail/Series/etc.), which already crossfade properly via `FadeGate`. Bumped the shared transition duration 150ms→200ms everywhere, AND made dashboard tab switches genuinely sequential — fade the old screen out first, only then swap to the new one (which starts invisible and fades in) — never both mounted at once, so the 2026-07-13 layout-squeeze regression (from an earlier, abandoned attempt at a true crossfade here) can't recur. Adds a small (~100ms) delay before a tab switch visibly starts, an accepted trade-off (confirmed via `AskUserQuestion`).
