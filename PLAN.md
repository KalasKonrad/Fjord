...
**Root cause pinned down and fixed, same day, via direct user follow-up.** User correctly identified the cause as a spun-down library drive taking ~15s+ to wake — confirmed exactly from the log's own timestamps (3 attempts × ~5s = 15.4s total before giving up). User then asked whether a genuine connection issue could be told apart from this using an already-existing signal (the WS's own keep-alive) rather than just waiting longer for everyone. Both fixed: `MAX_STALL_RELOAD_ATTEMPTS` split into a healthy-connection budget (7 attempts, ~40s — enough to ride out a slow drive wake) and an unhealthy/unknown one (2 attempts, ~15s — the original budget, so a genuine outage is still reported quickly) — chosen per-stall from a new live `FjordState.ws_connected`/`ws_last_keepalive_at` signal fed by the WebSocket's own already-running 30s keep-alive. Full design in CLAUDE.md's dated section.
