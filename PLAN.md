...
- [x] Bonfire Phase 5 (cross-household groups) — implemented 2026-08-29, `cargo build`/`clippy --workspace --all-targets`/`test --workspace` clean. Fixed the same day from live testing: a `BonfireGroupStatus` deserialize crash on open, the join-code field being completely untypeable with the on-screen keyboard disabled (plus a follow-on mistake where the Backspace half of that fix was dead code), a pre-existing (since Phase 2) shared-counter bug that visibly flashed the wrong Settings row whenever Enter was pressed inside this or Manage Profiles, and a toggle/label misalignment. **The core cross-household join actually worked, live-confirmed** between this dev machine's own two real master accounts — the single highest-priority item on the original checklist. One product question still open, asked of the user: should an owner also be able to join a different group at the same time (undocumented by the real API, untestable without a third account). See CLAUDE.md's own dated section for the full design and remaining checklist items (kick/leave propagation, hide-toggle round-trip, LAN-bypass admin warning, join rate-limit message).
