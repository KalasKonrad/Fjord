...
- [x] Bonfire Phase 5 (cross-household groups) — implemented 2026-08-29, `cargo build`/`clippy --workspace --all-targets`/`test --workspace` clean throughout. Fixed the same day from live testing: a `BonfireGroupStatus` deserialize crash on open, the join-code field being untypeable with the on-screen keyboard disabled (plus a follow-on Backspace regression), a shared-counter press-pulse leak, a toggle/label misalignment, then — from a real screenshot of Bonfire's own official UI proving owner+member can coexist — **two more urgent, confirmed bugs**: `sync_bonfire_subprofiles` was silently stealing another household's real sub-profiles into the wrong local account (a genuine data-corruption bug, root-caused and fixed — self-heals automatically), a related follow-up bug where a fellow group member's own sync could silently downgrade an already-independently-logged-in account into a PIN-gated group-routed one (fixed with a guard, doesn't self-heal — needs a fresh Add Account re-login for any account already corrupted by it), and switching into a cross-household account failed with a bare "400 Bad Request" that turned out to be a real, correct server-side requirement ("account has no PIN set") Fjord was simply discarding the body of and never showing (fixed generally across all 22 Bonfire API methods, not just this one). `BonfireGroupScreen` was then restructured from 3 mutually-exclusive states to 2 independent always-rendered sections (hosting + joining can now both be active), and both picker screens got visual-parity badges (a gold master-profile ring, a compact "LINKED" pill, a lock badge) — text/glyph-free where the screenshot's own crown/flame emoji turned out not to be covered by any bundled font. See CLAUDE.md's own dated section for the full design and the remaining live-verification checklist — none of this pass's fixes have been re-confirmed live yet.
