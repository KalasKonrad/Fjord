...
- **Bonfire/JellyProfiles integration — in progress, multi-session.** Full design approved and saved (plan file `groovy-stirring-phoenix.md`), documented in outline in this file's history and in full in CLAUDE.md as each phase lands. **Phase 0 (Settings screen data-driven rewrite) shipped 2026-08-07, first live keyboard walkthrough done the same day** (see CLAUDE.md's Settings section) — zero nav/dispatch bugs found in the rewrite itself; the one real bug the log surfaced (duplicate audio-device descriptions) was confirmed pre-existing and fixed anyway. Key Bindings' own row nav (previously undocumented and silent in the log) got UX fixes too: Reset-to-Defaults now shows focus/press feedback and requires confirmation, and a hint explains the rebind interaction. Debug logging substantially expanded per request. Build/clippy/test clean. **Still not live-tested**: mouse-click paths (only keyboard was walked through so far), the new duplicate-device-description fix, and the whole Key Bindings UX change (hint text, Reset confirm dialog, focus/press feedback) — none of Group 3 has been round-tripped through a rebuild yet. Phase 1 (Foundation + profile switching) is next, not yet started.
