...
- **Bonfire/JellyProfiles integration — in progress, multi-session.** Full design approved and saved (plan file `groovy-stirring-phoenix.md`), documented in outline in this file's history and in full in CLAUDE.md as each phase lands. **Phase 0 (Settings screen data-driven rewrite) shipped 2026-08-07, first live keyboard walkthrough done the same day** (see CLAUDE.md's Settings section) — zero nav/dispatch bugs found in the rewrite itself; the one real bug the log surfaced (duplicate audio-device descriptions) was confirmed pre-existing and fixed anyway. Key Bindings then got a real round of fixes: Reset-to-Defaults shows focus/press feedback and requires confirmation; key capture no longer cares about letter case/Caps Lock (was worked around via hand-doubled defaults, which is also why the screen showed labels like "F  f"); a real bug where `keybinding-rebinding` never reset after a successful rebind (leaving the next keypress silently consumed as another rebind attempt) is fixed; rebinding a key already used by a different action now requires confirmation instead of silently stealing it. Debug logging substantially expanded per request. Build/clippy/test clean. **Still not live-tested**: mouse-click paths through Settings generally (only keyboard walked through so far), the duplicate-device-description fix, and the whole Key Bindings rework (case/Caps-Lock normalization, the rebind-mode-stuck fix, both confirm dialogs) — none of it has been round-tripped through a rebuild yet. Phase 1 (Foundation + profile switching) is next, not yet started.
