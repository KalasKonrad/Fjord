...
- [x] **#28 — KDE dims screen / turns off display during playback** — Fixed: now calls both `org.freedesktop.ScreenSaver.Inhibit` and `org.kde.PowerManagement.Inhibition.Inhibit` at playback start; both are released at stop. KDE call is best-effort (silent no-op on non-KDE). `PlaybackCookies` struct holds both cookies; `tear_down_player` returns it. (`playback.rs`)
