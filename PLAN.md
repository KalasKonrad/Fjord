...
- [ ] Bonfire Phase 5 (cross-household groups) — implemented 2026-08-29, `cargo build`/`clippy --workspace --all-targets`/`test --workspace` clean. Fixed the same day from live testing: a `BonfireGroupStatus` deserialize crash on open, the join-code field being completely untypeable with the on-screen keyboard disabled, a pre-existing (since Phase 2) shared-counter bug that visibly flashed the wrong Settings row whenever Enter was pressed inside this or Manage Profiles, and a genuine mistake in the typing fix itself — the new Backspace handling was dead code, shadowed by an earlier unconditional Escape/Backspace-closes-screen check, so Backspace closed the whole screen instead of deleting a character. None of these 4 fixes yet re-confirmed live. The actual cross-household connect/switch flow is still entirely unverified (needs two real master accounts). See CLAUDE.md's own dated section for the full design, the real correctness bug found and fixed while designing it (a foreign group account was one code path away from being silently merged into the discovering master's own account, or from getting silently "repaired" back to a plain account on the next restart), and the priority-ordered live-verification checklist.
