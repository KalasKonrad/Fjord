...
- [ ] Stall-recovery connection-health split (2026-08-28) — needs the library drive actually spun down first (natural idle timeout, or force it with `hdparm -y`). Real risk flagged 2026-08-28: something else (smartd, updatedb/mlocate, Jellyfin's own library scan, a backup job, any systemd timer touching that mount) could re-wake the drive before Play is ever pressed, silently invalidating the test — confirm the drive is ACTUALLY still asleep right before playing with `hdparm -C /dev/sdX` (expect `drive state is: standby`), not just trust that it's been a few minutes since `-y`. Once genuinely confirmed asleep: a fresh play should ride out the wake automatically (no toast) under the healthy-connection budget. No easy way to test the "genuine outage still gives up in ~15s" half without an actual outage — leave that one to self-confirm if it ever happens, not worth chasing deliberately.
