...
- **Browse list** (`show-browse = true`, `active-nav == 3`): opens in sidebar mode (`current-item = -1`). In sidebar mode Up/Down navigate the sidebar normally; Right or Enter enters the list (`current-item = 0`). In list mode (`current-item >= 0`) Up/Down navigate items; Left or Up at top exits back to sidebar. Typing filters client-side; Enter plays selected item; Backspace/Escape closes and resets `active-nav = 0` when exiting via the Browse All sidebar entry. `B` shortcut also opens browse without changing `active-nav`.
