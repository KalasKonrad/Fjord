...
| `collection.rs` | `open_collection_screen(id, title, state, ww, rt)`: resets AppState collection props, sets `app-content-loading=true`, spawns async fetch of BoxSet items + all item posters + BoxSet poster + backdrop in parallel, single `invoke_from_event_loop` sets all data then shows page (`show-collection=true`); `handle_key`: grid nav (Up/Down/Left/Right, Enter→`open-detail`, C→context-menu), Back-button focus (Up from row 0), Back→close |
