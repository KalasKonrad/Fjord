...
3. **Floating mini-player** (`has-background-player + video-behind-ui = false`): `FloatingMiniPlayer` (320×90 px, bottom-right corner, top z-order) shows live video thumbnail + title + Resume/Stop. Visible on every screen. `N` key focuses it (`float-card-focused`); Left/Right toggle buttons, Enter activates, Back unfocuses.
...
- **Floating mini-player** (`has-background-player && !is-playing && !video-behind-ui`): `N` focuses it (`float-card-focused = 0`); Left/Right toggle between 0 (Resume) and 1 (Stop); Enter activates; Back unfocuses. Pre-dispatch check in `handle_key` intercepts these keys before the underlying screen. `reset_playback_ui` clears `float-card-focused = -1`.
