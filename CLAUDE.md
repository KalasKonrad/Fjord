...
│                               MusicPlayerBar (72px full-width bottom bar; is-audio-playing; left: art+title+artist; centre: ⏸/▶+⏹+progress+time; right: Phase 50 placeholder),
...
| `playback.rs` | `QueueItem { id, item_type, series_id, title }` — one entry in the playback queue. `VideoState` (incl. `chapters: Vec<(f64,String)>`, `chapters_loaded`, `chapter_load_attempts`, `chapter_osd_ticks`, `delay_osd_ticks`, `queue: VecDeque<QueueItem>`), `start_playback(url, id, item_type, title, config, client, series_id, audio_meta: Option<(artist, album_art_id)>, video, ww, rt)` — `audio_meta` drives music bar; when `item_type=="Audio"` sets `is-audio-playing=true` instead of `is-playing=true`, `fmt_secs`, `fmt_ends_at` (local wall-clock "ends at"), `build_track_model` (title→lang→codec label order; `external_filename` fallback for external subs), GL FBO helpers, `wire_rendering_notifier`, `wire_mpv_timer` (chapter polling after 2 s, up to 30 attempts; `chapter-marks` push; `chapter-entries` ([TrackEntry] id=index, label="M:SS  Title") push; `current-chapter` update per tick; chapter OSD countdown; delay OSD countdown; music-bar-pos/elapsed/total updated when `is-audio-playing`; queue pop on natural end when `!had_series`), `reset_playback_ui` (shared stop/natural-end UI reset; clears chapter-marks + chapter-entries + current-chapter + OSD + delay OSD + sub/audio-delay-ms + `is-audio-playing`) |
...
4. **Music bar** (`is-audio-playing = true`): full-width `MusicPlayerBar` (72 px) docked at the very bottom. Shown when audio-only items play (`item_type == "Audio"`). `is-playing` is **not** set — no fullscreen player opens. Left zone: 60×60 album art + title + artist (click → `music-bar-open-album`). Centre: ⏸/▶ + ⏹ + progress bar + elapsed/total. Right zone: placeholder for Phase 50 controls (Prev/Next/Shuffle/Repeat/♥/queue). `music-bar-play-pause` delegates to `pause_play_toggle`; `music-bar-stop` calls `do_stop_playback`.
