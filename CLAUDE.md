...
- **Collections library** (`LibraryGrid`, `active-nav == 3`): full poster grid of all BoxSets. No Unwatched/Favorites filters. Grid is always visible when this tab is active (no dashboard, no extra Enter press to reveal it); data fetch starts immediately on tab select. Right/Enter from sidebar enters the grid. Enter on a card opens `CollectionScreen`.
...
- **`-1` = sidebar**: Up/Down cycle nav tabs (0 Home → 1 TV Shows → 2 Movies → 3 Collections → 4 Music → 5 Browse All → 10 Settings → 11 Quit → wrap); arrowing to nav=5 opens `show-browse` immediately; nav=4 (Music) is a grayed placeholder — navigating to it does nothing; for nav=3 (Collections) `invoke_open_library(3)` is called automatically by `nav_to` so data starts loading immediately (no extra Enter required); Right/Enter enters the content grid or library; `settings-focused` is reset to -1 when `active-nav` changes and also when `B` opens browse.
