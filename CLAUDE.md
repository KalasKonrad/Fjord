...
- **Library grid** (`show-library = true`): 2D arrow nav across the poster grid; Enter opens detail; Backspace/Escape closes. Layout: top bar (54px) → sort bar (40px) → search field (52px) → poster grid with alphabet scrubber on right edge. Three focus states: **(1) grid mode** (`library-header-focused = false`, `library-sort-focused = false`) — arrow keys navigate posters, Up at row 0 focuses the search field, `/` also jumps to the search field, `Tab` toggles the sort bar focus; letter keys A–Z jump to that letter's first item (only when sort=Name A-Z and query empty, guard is encoded in the alpha-offsets being all-1 otherwise); **(2) search field focused** (`library-header-focused = true`) — letters type into query immediately, Backspace deletes (empty → back to grid), Down/Enter moves into results, Escape clears query and returns to grid; **(3) sort bar focused** (`library-sort-focused = true`) — Left/Right navigate cursor (0–4=sort pills, 5=Unwatched, 6=Favorites), Enter applies, `Tab`/Esc exits, Down exits to grid. Sort (0=Name A-Z, 1=Name Z-A, 2=Year↓, 3=Year↑, 4=Random) persisted per library type in `Config.library_movies_sort`/`library_series_sort`; filters reset to false on open. `refresh_library_display` in `browse.rs` is the single function that rebuilds `library-display` + `library-alpha-offsets` from current sort/filter/query. Alphabet scrubber: right-edge 22px strip, 27 cells (A-Z + #), visible only when sort=Name A-Z and no query; click or A-Z keyboard key calls `AppState.library-jump-to-letter(idx)` → reads `library-alpha-offsets[idx]` → sets `library-focused`.
