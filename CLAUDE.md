...
- **Series screen** (`show-series = true`): Full-page scrollable layout: backdrop → header (poster/title/meta/genres/overview) → Next Up SectionRow → season tabs → episode SectionRow → CastRow → More Like This SectionRow. Two nav states tracked by `series-in-season-row`: (1) **season row** (`true`) — Left/Right cycle tabs, Down/Enter enters episode row; (2) **episode row** (`false`) — Left/Right navigate cards in the horizontal SectionRow (`series-focused-ep`), Up returns to season row, Enter plays focused episode, I opens episode detail, C opens context menu. Backspace/Escape closes series screen and returns to previous screen.
