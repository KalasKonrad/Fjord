...
- **Mini-player bar** (`has-background-player && !is-playing`): Up from nav=0 in sidebar focuses it (`float-card-focused = 0`); Left/Right toggle between 0 (Resume) and 1 (Stop); Enter activates; Down/Back unfocuses. Pre-dispatch check in `handle_key` intercepts these keys before the underlying screen. `reset_playback_ui` clears `float-card-focused = -1`.
